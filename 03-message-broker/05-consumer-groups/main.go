...
const (
	spreadsheetsConsumerGroupName = "spreadsheets"
	notificationConsumerGroupName = "notifications"
)
...
func Subscribe(
	...
	orderConfirmationSub, err := redisstream.NewSubscriber(redisstream.SubscriberConfig{
		Client:        rdb,
		ConsumerGroup: notificationConsumerGroupName,
	}, logger)
	if err != nil {
		return err
	}
	...
	spreadsheetSub, err := redisstream.NewSubscriber(redisstream.SubscriberConfig{
		Client:        rdb,
		ConsumerGroup: spreadsheetsConsumerGroupName,
	}, logger)
	if err != nil {
		return err
	}
